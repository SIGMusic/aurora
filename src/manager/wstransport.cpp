...
int WSTransport::connect(int clientid) {
    ...
    char* buf = NULL;
    int recvlen = recv_complete(clientid, buf);
    if (recvlen == -1) {
        perror("recv");
        bad_request = 1;
    }
    ...
    if (!bad_request) {
        ...
        char* newkey = (char*)malloc(strlen(key) + sizeof(magic) + 1);
        strcpy(newkey, key);
        strcat(newkey, magic);
        char hash[SHA_DIGEST_LENGTH];
        SHA1((unsigned char*)newkey, strlen(newkey), (unsigned char*)hash);
        free(newkey);
        char* accept_token = base64((unsigned char*)hash, SHA_DIGEST_LENGTH);
...
ssize_t WSTransport::recv(int clientid, char* buf, size_t len) {
    ...
    int recvlen = ::recv(clientid, buf, len, 0);
    if (recvlen == -1) {
        perror("recv");
        return -1;
    }
...
ssize_t WSTransport::recv_complete(int sockfd, char*& buf) {
...
char* WSTransport::base64(const unsigned char *input, int length) {

    // Taken from http://www.ioncannon.net/programming/34/howto-base64-encode-with-cc-and-openssl/
